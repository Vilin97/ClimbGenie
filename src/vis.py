...
def draw_climb(triples, title=None, annotations=None):
...
def draw_climb_row(row):
    ...
    draw_climb(holds_xy, title=f"{row['name']} | {row['grade']} @ {int(row['angle'])}° | {int(row['ascents'])} ascents", annotations=holds_indices)
